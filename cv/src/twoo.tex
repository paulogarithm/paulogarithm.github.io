...
\begin{center}
	...
	\\[4pt]
	{\Large Ingénieur Logiciel Backend - Pédagogie \& Accompagnement}
	\\
\end{center}
...
Ingénieur logiciel backend (Go, Python, SQL, Docker), avec deux ans d'expérience d'accompagnement de {\bfseries plus de 400 étudiants} comme assistant technique à Epitech : pédagogie, écoute, création de contenus interactifs.
\\
Formation complémentaire en management à McGill University (Montréal), dont la sociologie du travail (Corporate Behaviour), et pratique professionnelle de l'anglais (C1).
\\
Contributeur open-source (plus de 10 000 lignes de code sur des packages critiques). En préparation de l'agrégation d'informatique.
\par\vspace{5pt}
...
\\
\hspace*{5pt}{\bfseries Transverses}: pédagogie, animation de cours, création de contenus interactifs, travail en équipe
\\
...
\begin{adjustwidth}{5pt}{0pt}
    Outil générique de migration multi-cloud gérant des structures de données complexes et des bases de données, avec une plateforme backend complète en Go et une couche d'abstraction cloud personnalisée ({\ttfamily provicol}) supportant AWS S3 et GCP. \\
    Utilisation de l'API Docker à l'exécution pour orchestrer dynamiquement des conteneurs d'environnements concurrents.
\end{adjustwidth}
