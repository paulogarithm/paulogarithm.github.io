...
Ingénieur logiciel spécialisé en C/C++ et programmation bas niveau (assembleur, gestion mémoire, réseau, Linux), attentif à la fiabilité du code (tests, Valgrind, CI/CD), avec un intérêt pour la sécurité et le reverse engineering.
\\
Un an d'études à Montréal (McGill University, management) et pratique professionnelle de l'anglais (C1).
\\
...
\\
\hspace*{5pt}{\bfseries Sécurité}: CTF (Hack The Box, TryHackMe), reverse engineering (gdb, Ghidra)
\\
...
\begin{itemize}[noitemsep, topsep=0pt]
	\item Développement de 6+ packages core en Gnolang (variante de Go pour smart contracts), dont une compatibilité EVM.
...
\begin{adjustwidth}{5pt}{0pt}
    Conception / codage d'un serveur TCP mono-thread à boucle événementielle. Gestion des clients par liste chaînée de connexions. \\
...
\par
\begin{adjustwidth}{5pt}{0pt}
	Architecture ECS orientée données : {\bfseries 100k entités animées}, $\sim$6 ms de moteur et $\sim$6 ms de rendu par image ; alignement mémoire et extensions SIMD du processeur.
\end{adjustwidth}
\par
